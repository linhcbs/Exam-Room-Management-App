...
const original_formats = {
    "mẫu 1": [1, 2, 3, 4, 5, 6, 12, 11, 10, 9, 8, 7, 13, 14, 15, 16, 17, 18, 24, 23, 22, 21, 20, 19, 25, 26, 27, 28, 29, 30, 36, 35, 34, 33, 32, 31],
    "mẫu 2": [1, 2, 3, 19, 20, 21, 6, 5, 4, 24, 23, 22, 7, 8, 9, 25, 26, 27, 12, 11, 10, 30, 29, 28, 13, 14, 15, 31, 32, 33, 18, 17, 16, 36, 35, 34],
    "mẫu 3": [1, 12, 13, 24, 25, 36, 2, 11, 14, 23, 26, 35, 3, 10, 15, 22, 27, 34, 4, 9, 16, 21, 28, 33, 5, 8, 17, 20, 29, 32, 6, 7, 18, 19, 30, 31],
    "mẫu 4": [1, 21, 22, 23, 24, 7, 20, 2, 33, 34, 8, 25, 19, 32, 3, 9, 35, 26, 18, 31, 10, 4, 36, 27, 17, 11, 30, 29, 5, 28, 12, 16, 15, 14, 13, 6]
}
...
function renderFormatInfo() {
    let formats = JSON.parse(window.localStorage.getItem("room-id-formats-36"));
    let i = 1;
...
function renderFormatByOption(e) {
    ...
    let format = JSON.parse(window.localStorage.getItem("room-id-formats-36"))[e];
    setValuesForInputs(format)
}

function setNull() {
    if (localStorage.getItem("room-id-formats-36") === null || localStorage.getItem("room-id-formats-36") == "{}") {
        window.localStorage.setItem("room-id-formats-36", JSON.stringify(original_formats))
    }
}
...
function renderFormatOptions(id) {
    ...
    setNull();
    let format = JSON.parse(window.localStorage.getItem("room-id-formats-36"))
...
function saveFormat() {
    ...
    //console.log(formats)
    alert("Đã lưu");
    window.localStorage.setItem("room-id-formats-36", JSON.stringify(formats))
